...
export default class GraphQLWhere {
  ...
  constructor(filterObj: Filter) {
    const weaviateFilterObj = GraphQLWhere.transformFilter(filterObj);
    this.source = weaviateFilterObj;
  }

  static transformFilter(filter: Filter): WhereFilter {
    const { conditions, property, ...rest } = filter;
    const weaviateFilter: WhereFilter = {
      ...rest,
      ...(property && { path: property }),
      ...(conditions && { operands: conditions.map(GraphQLWhere.transformFilter) }),
    };
